...
let hisData = JSON.parse(localStorage.getItem('treeData'))
    ...
    createClass(RackBuilder, [{
        ...
        value: function init() {
                var self = this,
                    ...
                    splitView = this.splitView = new ht.widget.SplitView(listBorder, borderPane, 'h', 0);
        ...
        value: function handleGraphInteractor(e) {
            ...
            if (e.kind === 'prepareMove') {
                ...
                self._oldPanelId = target._parent._attrObject.id
                ...
                self.showDragHelper(targetPoint, uHeight, target._attrObject.id, '#7ED321');
                dm.sendToTop(target);
                ...
                var rack = self.showDragHelper(targetPoint, uHeight, target._attrObject.id, '#7ED321');

                if (dm.contains(self.dragHelper) && rack[1]) { // 判断容器是否包含该 data 对象
                    let sindex = rack[0].getCellIndex(self.dragHelper.p().y) - uHeight + 1;
                    target.a('uindex', sindex); // 设置节点的业务属性 uindex
                    target.p(self.dragHelper.p()); // 设置节点的坐标

                    dm.remove(self.dragHelper); // 移除
                    self._savable = true;
                    self.toolbar.iv();
                    target.setHost(rack[0]); // 设置宿主节点
                    target.setParent(rack[0]); // 设置父亲节点
                    // update tree
                    // var treeNode = target.a(rack[0].__treeNode); // 获取拓扑图上对应的树上的节点
                    // treeNode.setParent(rack[0].a(rack[0].__treeNode));
                    // this.setPaneTooltip(target);
                    var treeNodes = target.__treeNode;
                    treeNodes.setParent(rack[0].__treeNode);
                    // this.setPsaneTooltip(target);

                    var treeNode = target.a(rack[0].__treeNode);

                    if (target._parent._attrObject.id == self._oldPanelId) {
                        // 设备未切换机架s
                        searchPos(treeData, target._parent._attrObject.id, treeNode._attrObject.id, treeNode._attrObject.uindex)
                    } else {
                        //设备切换机架
                        let obj = { id: treeNode._attrObject.id, name: treeNode._attrObject.name, type: treeNode._attrObject.type, uHeight: treeNode._attrObject.uHeight, uindex: treeNode._attrObject.uindex, img: treeNode._attrObject.img, pid: target._parent._attrObject.id }
                            // 查找原位置删除
                        searchOldPos(treeData, self._oldPanelId, treeNode._attrObject.id)
                            //放入新的机柜
                        deepFor(treeData, target._parent._attrObject.id, obj)
                    }
                } else {
        ...
        value: function handleListDND(e, state) {
            ...
            if (state === 'prepare') {
                ...
                if (dnd) {
                    ...
                    dnd.style.left = pagePoint.x - dnd.offsetWidth * 0.5 + 'px';
                    dnd.style.top = pagePoint.y - dnd.offsetHeight * 0.5 + 'px';
                    let ld = self.listView.sm().ld();
                    let ldgetAttrObject = ld.getAttrObject();

                    self.showDragHelper(e, ldgetAttrObject.uHeight, ids, '#7ED321');
                }
            } else {
                var rect = self.dragHelper.getRect(),
                    node,
                    ld = self.listView.sm().ld(),
                    ldgetAttrObject = ld.getAttrObject();
                ldgetAttrObject.id = ids

                if (ht.Default.containedInView(e, self.gv)) {
                    if (dm.contains(self.dragHelper)) {
                        var target = self.showDragHelper(e, ldgetAttrObject.uHeight, ids, '#7ED321')
                        if (target[1]) {
                            let uindex = target[0].getCellIndex(rect.y, ld.a('uHeight') || 1);
                            node = self.createPane(rect, ldgetAttrObject, target[0], uindex);
                            dm.add(node);
                            // update tree data
                            var treeNode = self.main.createData(ldgetAttrObject, target[0].__treeNode);
                            treeNode.a('uindex', uindex);

                            node.__treeNode = treeNode;
                            let datas = JSON.parse(JSON.stringify(node._attrObject))
                            deepFor(treeData, node._parent._attrObject.id, datas)
                            dm.remove(self.dragHelper);
                        } else {
                            dm.remove(self.dragHelper);
                        }
        ...
        key: "showDragHelper",
        value: function showDragHelper(e, uHeight, dragId, color) {
            var self = this,
                dm = self.gv.dm(),
            ...
            if (targetRack) {
                let uidnex = targetRack.getCellIndex(rect.y, uHeight)
                let orgData = targetRack._children._as
                let s1 = uidnex,
                    e1 = uidnex + uHeight - 1;
                for (let i = 0; i < orgData.length; i++) {

                    if (orgData[i]._attrObject.id != dragId) {
                        //查询是否有重复
                        let s2 = orgData[i]._attrObject.uindex
                        let e2 = orgData[i]._attrObject.uindex + orgData[i]._attrObject.uHeight - 1
                        var max = [s1, s2];
                        var min = [e1, e2];
                        // console.log(s1, '<', e1, 'and', s2, '<', e2)
...
var deepFor = function(arr, parentId, data) {
    ...
    if (arr instanceof Array) {
        for (let i = 0; i < arr.length; i++) {
            ...
            if (arr[i].children && arr[i].children.length > 0) {
                deepFor(arr[i].children, parentId, data)
            }
...
var searchPos = function(arr, parentId, sbid, uindex) {
    if (arr instanceof Array) {
        for (let i = 0; i < arr.length; i++) {
            if (arr[i].id == parentId) {
                let jg = arr[i].children
            ...
            if (arr[i].children && arr[i].children.length > 0) {
                searchPos(arr[i].children, parentId, sbid, uindex)
            }
        }
    } else {
        return
...
var searchOldPos = function(arr, parentId, sbid) {
    if (arr instanceof Array) {
        for (let i = 0; i < arr.length; i++) {
            if (arr[i].id == parentId) {
                let jg = arr[i].children
            ...
            if (arr[i].children && arr[i].children.length > 0) {
                searchOldPos(arr[i].children, parentId, sbid)
            }
        }
    } else {
        return
...
var reMoveNode = function(arr, deleteId) {
    if (arr instanceof Array) {
        for (let i = 0; i < arr.length; i++) {
            ...
            if (arr[i].children && arr[i].children.length > 0) {
                reMoveNode(arr[i].children, deleteId)
            }
        }
    } else {
        return
...
var treeToJson = function(data) {
    return data.reduce((arr, { children = [], id, img, name, pid, uindex, uHeight, usize, type }) =>
        arr.concat([{ id, img, name, pid, uindex, uHeight, usize, type }], treeToJson(children)), [])
...
function update(params) {
    let jdata = treeToJson(treeData)
    $.ajax({
        type: 'POST',
        url: 'http://10.163.228.54:3000/api/croom/addData',
        ...
        success(res) {
            layui.use(['layer'], function() {
                ...
                layer.msg('保存成功！', { icon: 7, time: 2000, shade: [0.6, '#000', true] });
            });
        }
    })
}
    ...
    createClass(Main, [{
        ...
        value: function initUI() {
            ...
            var treeView = this.treeView = new ht.widget.TreeView(),
                splitView = this.splitView = new ht.widget.SplitView(treeView, null, 'h', 0);
            ...
            treeView.sm().ms(function() {
                ...
                if (ld != null && ld._attrObject.type != 9) {
                    _this.updateForm(ld.a('type'));
                } else {
                    // _this.splitView.setRightView(null);
                }
            });
            // treeView.getRowDatas = function(params) {
        ...
        value: function loadTreeData() {
            var self = this;
            let p = getQueryVariable('id')
            $.ajax({
                    type: 'GET',
                    url: 'http://10.163.228.54:3000/api/croom/getRackData?id=' + p,
                    contentType: "application/json;charset=UTF-8",
                    data: {},
                    success(res) {
